...
## < 미션 종료됨 >
총 소요 시간: **4시간 52분**  
모든 기능 구현을 완료했으나, 시간이 부족하여 테스트 코드 작성 및 리팩토링은 진행하지 못함.  
...
### 페어 매칭 내역을 관리하는 역할
- [x] 각 레벨별, 미션별 페어 매칭 내역을 효과적으로 저장한다.
- [x] 그것을 관리하는 다양한 메서드를 포함하고 있다.

### 단일 크루 역할
- [x] 크루의 Course 정보를 필드에 가지고 있다.
- [x] 크루의 이름 정보를 필드에 가지고 있다.

### 매칭된 페어 크루를 저장하는 역할
- [x] 매칭된 크루를 저장한 자료구조(아마 2차원 배열)를 필드에 가지고 있다.
- [x] 이와 관련된 메서드를 포함하고 있다.
